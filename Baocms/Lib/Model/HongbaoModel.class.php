...
class HongbaoModel extends CommonModel
{
    ...
    public function createhongbao($money,$bom_num,$num,$roomid,$uid){
        ...
        //开始作弊喽  不是机器人就给他降雷
        $userinfo=D('Users')->getUserByUid($uid);
        if($userinfo['is_robot']==0){
            $this->resetArr($kickarr,$bom_num);
        }
        //end
    ...
    private function resetArr(&$kickarr,$bom_num){
        //获取雷的个数
        $bom_nums=0;
        foreach ($kickarr as $kk=> $vs){
            if($kk>0){
                if($bom_num==substr((int)$vs,-1)){
                    $bom_nums++;
                }
            }
        ...
        if($bom_nums==1){
            $rand=rand_string(2,1);
            if($rand>49){
                foreach ($kickarr as $k1=> &$v1){
                    if($k1>0){
                        if($bom_num==substr((int)$v1,-1)){
                            if($v1>1){
                                $v1=$v1-1;
                                break;
                            }
            ...
            foreach ($kickarr as $k=> &$v){
                if($k>0){
                    if($bom_num==substr((int)$v,-1)){
                        if($v>1){
                            $v=$v-1;
                            break;
                        }
